...
class LoRAConv2d(nn.Module):
    ...
    def __init__(self, base_module: nn.Conv2d, lora_config: dict) -> None:
        ...
        for parameter in self.base_module.parameters():
            parameter.requires_grad = False
        out_channels, in_channels, kernel_height, kernel_width = self.base_module.weight.size()
        ...
        if self.rank_for == 'kernel':
            self.delta_weight_A = nn.Parameter(
                torch.empty(
                    size=(out_channels, in_channels, kernel_height, lora_config['rank']),
                    dtype=self.base_module.weight.dtype,
                    device=self.base_module.weight.device
                )
            )
            self.delta_weight_B = nn.Parameter(
                torch.empty(
                    size=(out_channels, in_channels, lora_config['rank'], kernel_width),
                    dtype=self.base_module.weight.dtype,
                    device=self.base_module.weight.device
                )
            )
            ...
            self.delta_weight_A = nn.Parameter(
                torch.empty(
                    size=(kernel_height, kernel_width, out_channels, lora_config['rank']),
                    dtype=self.base_module.weight.dtype,
                    device=self.base_module.weight.device
                )
            )
            self.delta_weight_B = nn.Parameter(
                torch.empty(
                    size=(kernel_height, kernel_width, lora_config['rank'], in_channels),
                    dtype=self.base_module.weight.dtype,
                    device=self.base_module.weight.device
                )
            )
    ...
    def __repr__(self) -> str:
        ...
        if self.rank_for == 'kernel':
            adapter_repr_string = f'Adapter(kernel_height={kernel_height}, rank={self.rank.item()}, kernel_width={kernel_width})'
        elif self.rank_for == 'channels':
